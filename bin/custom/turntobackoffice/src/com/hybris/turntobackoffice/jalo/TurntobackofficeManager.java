 ...
 */
public class TurntobackofficeManager extends GeneratedTurntobackofficeManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(TurntobackofficeManager.class.getName());
	 ...
	 */
	public static TurntobackofficeManager getInstance()
	{
		return (TurntobackofficeManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
				.getExtension(TurntobackofficeConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public TurntobackofficeManager() // NOPMD
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of TurntobackofficeManager called.");
		}
	}
