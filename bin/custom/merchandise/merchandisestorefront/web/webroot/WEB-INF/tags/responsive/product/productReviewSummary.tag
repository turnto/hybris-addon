...
<c:set var="isOnQA" value="${flags.get('checkboxQA').getFlag() eq 'true'}"/>
...
<c:set var="isOnCustomerGallery" value="${flags.get('customerGallery').getFlag() eq 'true'}"/>


<c:set var="isTurnOnSomeComponent" value="${isOnQA or isOnRating or isOnCustomerGalleryRowWidget or isOnTurntoCheckoutChatter or isOnCustomerGallery}"/>
...
<c:set var="isOverlayRating" value="${isOnRating and flags.get('checkboxRating').getSetupType().getCode() eq \"overlay\"}"/>
...
<c:if test="${isTurnOnSomeComponent}">
